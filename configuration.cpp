    ...
    ui(new Ui::Configuration)
{
    ...
    qry->exec("select co.keys, co.location, em.name from configuration as co inner join company as em on co.rut_company=em.rut where co.keys=(select keys from users where rut = '"+rutSignin+"')");
    while(qry->next()){
    ...
    qry->exec("select name as Nombre,serial as Serial,baudrate as Baudrate from devices where keys=(select keys from users where rut = '"+rutSignin+"')");
    ...
    if(conn.getFirstFromDb(rutSignin, "select output_register from configuration where keys=(select keys from users where rut = '"+rutSignin+"')") == "YES"){
...
void Configuration::on_pushButton_save_clicked()
{
    ...
    if(!qry->exec("UPDATE configuration SET location = '"+ui->lineEdit_place->text()+
                  ...
                  "' where keys='"+ui->lineEdit_key->text()+"'"))
    {
